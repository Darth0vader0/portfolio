...
const Portfolio = () => {
  ...
  // Define scroll sections and their transforms
  const sections = [
    { name: 'overview', scale: 1, x: 0, y: 0, range: [0, 0.1] },
    { name: 'profile', scale: 3, x: -1729, y: -850, range: [0.1, 0.25] },
    { name: 'skills', scale: 2.5, x: -2200, y: -650, range: [0.25, 0.4] },
    { name: 'projects', scale: 2.8, x: -1500, y: -1300, range: [0.4, 0.55] },
    { name: 'work', scale: 2.2, x: -2300, y: -1500, range: [0.55, 0.7] },
    { name: 'education', scale: 2.2, x: -2580, y: -1500, range: [0.7, 0.85] },
    { name: 'bye', scale: 2.5, x: -1800, y: -1700, range: [0.85, 1] }
  ];

  // Create smooth transforms for each section
  const scale = useTransform(scrollYProgress, 
    sections.map(s => s.range[0]), 
    sections.map(s => s.scale)
  );
  ...
  useEffect(() => {
    const unsubscribe = scrollYProgress.onChange((latest) => {
      const section = sections.find(s => latest >= s.range[0] && latest <= s.range[1]);
      if (section) {
  ...
  return (
    <div className="w-full min-h-[800vh] bg-gradient-to-b from-slate-900 via-blue-900 to-slate-800">
      ...
      <div className="fixed inset-0 overflow-hidden">
        ...
        >
          <svg
            ...
            onMouseEnter={(e) => {
              const target = e.target.closest('path, rect');
              if (target && target.getAttribute('data-section')) {
                setHoveredElement(target.getAttribute('data-section'));
              }
            }}
            ...
            onClick={(e) => {
              const target = e.target.closest('path, rect');
              if (target && target.getAttribute('data-section')) {
                setClickedElement(target.getAttribute('data-section'));
              }
            }}
          >
            <defs>
              ...
              {/* Glowing effect for active sections */}
              <filter id="glow">
            ...
            <g>
              ...
              <path
                className={`fill-emerald-500 cursor-pointer transition-all duration-300 ${
                  hoveredElement === 'work' || currentSection === 'work' 
                    ? 'opacity-90 drop-shadow-lg' 
                    : 'opacity-60'
                }`}
                d="m 2336.2831,1650.5269 1.3763,-369.5484 -348.9032,95.6559 -8.1432,330.9398 z"
                data-section="work"
                style={currentSection === 'work' ? { filter: 'url(#glow)' } : {}}
              >
              ...
              <path
                className={`fill-emerald-500 cursor-pointer transition-all duration-300 ${
                  hoveredElement === 'education' || currentSection === 'education'
                    ? 'opacity-90 drop-shadow-lg' 
                    : 'opacity-60'
                }`}
                d="m 2584.3579,1670.4782 -1.3763,-369.5484 348.9032,95.6559 8.1432,330.9398 z"
                data-section="education"
                style={currentSection === 'education' ? { filter: 'url(#glow)' } : {}}
              >
              ...
              <path
                className={`fill-white cursor-pointer transition-all duration-300 ${
                  hoveredElement === 'profile' || currentSection === 'profile'
                    ? 'drop-shadow-2xl' 
                    : 'drop-shadow-lg'
                }`}
                d="m 1729.1726,850.10963 0.1217,-32.7246 c 0,0 1.4598,-24.33056 26.0337,-29.31832 24.5738,-4.98777 29.4399,11.43536 29.4399,11.43536 l 0.365,55.71697 0.8515,72.26175 0.8515,40.02377 c 0,0 -2.433,33.69784 -31.3864,39.29384 -28.9534,5.596 -25.1821,-49.39102 -25.1821,-49.39102 z"
                data-section="profile"
                style={currentSection === 'profile' ? { filter: 'url(#glow)' } : {}}
              />

              {/* Cat Picture */}
              <path
                className="fill-orange-400 drop-shadow-md"
              ...
              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                <path
                  key={`skill${num}`}
                  className={`fill-blue-400 cursor-pointer transition-all duration-300 ${
                    hoveredElement === `skill${num}` || currentSection === 'skills'
                      ? 'opacity-90 drop-shadow-lg' 
                      : 'opacity-70'
                  }`}
                  d={getSkillPath(num)}
                  data-section={`skill${num}`}
                  style={currentSection === 'skills' ? { filter: 'url(#glow)' } : {}}
                />
              ))}
              ...
              <path
                className={`fill-white cursor-pointer transition-all duration-300 ${
                  hoveredElement === 'workLogo' || currentSection === 'work'
              ...
              <path
                className={`fill-white cursor-pointer transition-all duration-300 ${
                  hoveredElement === 'eduLogo' || currentSection === 'education'
                    ? 'drop-shadow-xl scale-110' 
                    : 'drop-shadow-lg'
                }`}
              ...
              {[1, 2, 3, 4, 5, 6].map((num) => (
                <path
                  key={`extra${num}`}
                  className={`fill-purple-400 cursor-pointer transition-all duration-300 ${
                    hoveredElement === `extra${num}` ? 'opacity-80' : 'opacity-60'
                  }`}
                  d={getExtraPath(num)}
                  data-section={`extra${num}`}
                />
              ))}

              {/* Bye Section */}
              <path
                className={`fill-rose-400 transition-all duration-300 ${
                  currentSection === 'bye' ? 'drop-shadow-2xl' : 'drop-shadow-md'
                }`}
                ...
                data-section="bye"
                style={currentSection === 'bye' ? { filter: 'url(#glow)' } : {}}
              />

              {/* Interactive arrows and banners */}
              <path
                className={`fill-emerald-400 opacity-80 cursor-pointer transition-transform duration-200 ${
                ...
                data-section="downArrow"
              />

              <path
                className={`fill-emerald-400 cursor-pointer transition-transform duration-200 ${
                ...
                data-section="upArrow"
              />

              {/* Project Banner */}
              <path
                className={`fill-emerald-400 transition-all duration-300 ${
                  currentSection === 'projects' ? 'opacity-90 drop-shadow-lg' : 'opacity-70'
                }`}
              ...
              {/* Other elements */}
              <path className="opacity-100 fill-slate-300" d="m 1680.1465,1367.3772 61.5563,-3.5279 0.4866,31.3864 -62.1646,4.9878 z" />
              <path className="opacity-100 fill-slate-300" d="m 1805.8138,1791.4588 53.0406,4.1362 0.9732,39.1722 -54.7437,-3.163 z" />
              <path className="opacity-100 fill-slate-300" d="m 1701.9223,1716.2774 c 35.036,-0.4866 35.036,-0.4866 35.036,-0.4866 l 0.73,67.6389 c -35.036,0.2433 -35.036,0.2433 -35.036,0.2433 z" />

              {/* Project Details Rectangle */}
              <rect
                className={`fill-emerald-400 cursor-pointer transition-all duration-300 rounded-lg ${
                  hoveredElement === 'projectDetails' || currentSection === 'projects'
                ...
                ry="19.595598"
                rx="19.595598"
                data-section="projectDetails"
                style={currentSection === 'projects' ? { filter: 'url(#glow)' } : {}}
              />

              {/* Skill Banner */}
              <path
                className={`fill-blue-500 cursor-pointer transition-all duration-300 ${
                  hoveredElement === 'skillBanner' || currentSection === 'skills'
                    ? 'opacity-80 drop-shadow-lg' 
                    : 'opacity-60'
                }`}
                d="m 2160.172,578.75267 v 128 l 290.4086,68.12903 3.4408,-116.30107 z"
                data-section="skillBanner"
                style={currentSection === 'skills' ? { filter: 'url(#glow)' } : {}}
              >
              ...
              {/* Adaptive Text */}
              <path
                className={`fill-cyan-400 cursor-pointer transition-all duration-300 ${
              ...
              <path
                className={`fill-yellow-400 cursor-pointer transition-all duration-300 ${
                ...
                data-section="clickEduWork"
              />

              {/* Project Window */}
              <path
                className={`fill-emerald-500 cursor-pointer transition-all duration-300 ${
                  hoveredElement === 'projectWindow' || currentSection === 'projects'
                    ? 'opacity-80 drop-shadow-lg' 
                    : 'opacity-60'
                }`}
      ...
      {hoveredElement && (
        <div className="fixed top-4 left-4 bg-black bg-opacity-75 text-white px-4 py-2 rounded-lg z-10 font-medium shadow-lg backdrop-blur-sm">
          ...
          <span className="text-emerald-400 ml-1 font-semibold">{hoveredElement}</span>
        </div>
      )}

      {clickedElement && (
        <div className="fixed bottom-4 right-4 bg-emerald-500 bg-opacity-90 text-white px-4 py-2 rounded-lg z-10 font-medium shadow-lg backdrop-blur-sm animate-pulse">
      ...
      <div className="fixed top-4 right-4 bg-black bg-opacity-75 text-white px-4 py-2 rounded-lg z-10 font-medium shadow-lg backdrop-blur-sm">
        <span className="text-sm">Current:</span>
        <span className="text-emerald-400 ml-1 font-semibold capitalize">{currentSection}</span>
      </div>

      {/* Scroll Progress Bar */}
      <motion.div 
        className="fixed top-0 left-0 right-0 h-1 bg-emerald-500 z-20 origin-left"
          ...
          <div
            ...
            className={`w-3 h-3 rounded-full border-2 transition-all duration-300 ${
              currentSection === section.name
                ...
                : 'bg-transparent border-white/50 hover:border-emerald-400'
            }`}
          />
        ))}
      </div>
      ...
      {currentSection === 'overview' && (
        <motion.div 
          ...
          transition={{ delay: 1 }}
        >
          <div className="bg-black bg-opacity-50 px-6 py-3 rounded-full backdrop-blur-sm">
            <p className="text-lg font-medium">Scroll to explore my portfolio</p>
            <motion.div 
